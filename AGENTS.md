...
- **Backend/auth/storage require env vars that are set up separately** (not committed): copy `.env.example` → `.env.local`. `pnpm convex dev` writes `VITE_CONVEX_URL`/`CONVEX_DEPLOYMENT` locally but requires a Convex login (device auth). Clerk (`VITE_CLERK_PUBLISHABLE_KEY`, `CLERK_JWT_ISSUER_DOMAIN`) and Cloudflare R2 uploads (`R2_*`) also need real credentials. Don't expect these services to run without those values.
