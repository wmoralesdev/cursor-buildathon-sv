...
type Props = {
  ...
  slateActive: boolean;
  omitSlateActiveAttribute?: boolean;
};
...
export const WelcomeCardSponsorSlate = memo(
  forwardRef<HTMLDivElement, Props>(function WelcomeCardSponsorSlate(
    { aspectFormat, slateActive, omitSlateActiveAttribute = false },
    ref,
  ) {
  const layout = FORMAT_LAYOUT[aspectFormat];

  const sponsorTotal = WELCOME_CARD_SPONSOR_MARK_COMPONENTS.length;
            ...
            <div style={postWallOuterStyle}>
              {postRowIndices.map((indices, rowIdx) => (
                <div key={rowIdx} style={postRowStyle}>
                  {indices.map((index) => {
                    const Mark = WELCOME_CARD_SPONSOR_MARK_COMPONENTS[index];
                    return (
                      <Mark
